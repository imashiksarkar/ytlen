...
import type { AxiosResponseType } from "./main.d"
...
class Youtube {
  ...
  private async getVideoIds(playlistId: string): Promise<string[] | null> {
  ...
  private totalLengthInSeconds(durationStr: string) {
    const regEx = /\d+[HMS]/g
